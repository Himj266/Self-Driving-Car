...
class Dqn():
    ...
    def select_action(self, state):
        ...
        probs = F.softmax(self.model(Variable(state, volatile = True))*100)   # Temperature coff = 100 so that we can have better look at each option.(At Temperature coff -> infinity the probablity for each action tend to equalize and if temperature coff -> 0 probability for each action tend to shift towards one on the action)
    ...
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        ...
        target = self.gamma*next_outputs + batch_reward                                 # calculating theoritical q value i.e q(s(t)) = r(s,a) + gamma*max(q(a,s(t+1))) for every action possible
        td_loss = F.smooth_l1_loss(outputs, target)                                         # Loss = actual - predicted
        self.optimizer.zero_grad()                                                           # Reinitializing the optimizer at each iteration of loop
    ...
    def update(self, reward, new_signal):
        ...
        self.memory.push((self.last_state, new_state, torch.LongTensor([int(self.last_action)]), torch.Tensor([self.last_reward])))    # Updating memory after reaching new state
